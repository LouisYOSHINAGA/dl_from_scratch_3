...
class NegTest(unittest.TestCase):
    ...
    def test_backward(self) -> None:
        ...
        y: Variable = -x
        y.backward()
        expected: np.array = np.array(-1.0)
...
class SubTest(unittest.TestCase):
    ...
    def test_backward(self) -> None:
        ...
        y: Variable = x0 - x1
        y.backward()
        expected0: np.ndarray = np.array(1.0)
        expected1: np.ndarray = np.array(-1.0)
    ...
    def test_forward_twice(self) -> None:
        ...
        x = Variable(rng.random(1))
        y: Variable = x - x
        expected: np.ndarray = np.array(0.0)
        self.assertEqual(y.data, expected)
...
class DivTest(unittest.TestCase):
    def test_forward(self) -> None:
        ...
        y: Variable = x0 / x1
        expected: np.ndarray = np.array(1.5)
        self.assertEqual(y.data, expected)
    ...
    def test_backward(self) -> None:
        ...
        y: Variable = x0 / x1
        y.backward()
        expected0: np.ndarray = np.array(0.5)
        expected1: np.ndarray = np.array(-0.75)
        self.assertEqual(x0.grad, expected0)
        self.assertEqual(x1.grad, expected1)
...
class PowTest(unittest.TestCase):
    def test_forward(self) -> None:
        ...
        c: Scalar = 3.0
        y: Variable = x ** c
        expected: np.ndarray = np.array(8.0)
        self.assertEqual(y.data, expected)
    ...
    def test_backward(self) -> None:
        ...
        expected: np.ndarray = np.array(8.0)
        y.backward()
        expected: np.ndarray = np.array(12.0)
        self.assertEqual(x.grad, expected)
