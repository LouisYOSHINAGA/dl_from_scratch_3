...
class MomentumSGD(Optimizer):
    def __init__(self, lr: float =0.01, momentum: float =0.9) -> None:
        ...
        self.vs: dict[int, xpndarray] = {}

    def update_one(self, param: Parameter) -> None:
        v_key: int = id(param)
        if v_key not in self.vs:
            self.vs[v_key] = xpy.zeros_like(param.data)
        v: xpndarray = self.momentum * self.vs[v_key] - self.lr * param.grad.data
        param.data += v
...
class AdaGrad(Optimizer):
    def __init__(self, lr: float =0.01, eps: float =1e-8) -> None:
        ...
        self.lr: float = lr
        self.eps: float = eps
        self.hs: dict[int, xpndarray] = {}

    def update_one(self, param: Parameter) -> None:
        h_key: int = id(param)
        if h_key not in self.hs:
            self.hs[h_key] = xpy.zeros_like(param.data)
        self.hs[h_key] += param.grad.data ** 2
        param.data -= self.lr * param.grad.data / (xpy.sqrt(self.hs[h_key] + self.eps))
...
class AdaDelta(Optimizer):
    def __init__(self, rho: float =0.95, eps: float =1e-6) -> None:
        ...
        self.rho: float = rho
        self.eps: float = eps
        self.msg: dict[int, xpndarray] = {}
        self.msdx: dict[int, xpndarray] = {}

    def update_one(self, param: Parameter) -> None:
        key: int = id(param)
        if key not in self.msg:
            self.msg[key] = xpy.zeros_like(param.data)
            self.msdx[key] = xpy.zeros_like(param.data)
        self.msg[key] = self.rho * self.msg[key] + (1 - self.rho) * param.grad.data ** 2
        dx: float = xpy.sqrt((self.msdx[key] + self.eps) / (self.msg[key] + self.eps)) * param.grad.data
        param.data -= dx
...
class Adam(Optimizer):
    ...
    @property
    def lr(self) -> float:
        fix1: float = 1 - xpy.power(self.beta1, self.t)
        fix2: float = 1 - xpy.power(self.beta2, self.t)
        return self.alpha * xpy.sqrt(fix2) / fix1

    def update_one(self, param: Parameter) -> None:
        key: int = id(param)
        if key not in self.ms:
            self.ms[key] = xpy.zeros_like(param.data)
            self.vs[key] = xpy.zeros_like(param.data)
        ...
        param.data -= self.lr * self.ms[key] / (xpy.sqrt(self.vs[key]) + self.eps)